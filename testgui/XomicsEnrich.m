%%%%%%%%%%%%%%%%%%%%%%%%%%% 主函数 %%%%%%%%%%%%%%%%%%%%%%%%%%
function varargout = XomicsEnrich(varargin)
% TEST1 MATLAB code for XomicsEnrich.fig
...
%      applied to the GUI before XomicsEnrich_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to XomicsEnrich_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help XomicsEnrich
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @XomicsEnrich_OpeningFcn, ...
                   'gui_OutputFcn',  @XomicsEnrich_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% openingfucntion %%%%%%%%%%%%%%%%%%%%%%%%%%
% --- Executes just before XomicsEnrich is made visible.
function XomicsEnrich_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to XomicsEnrich (see VARARGIN)

% Choose default command line output for XomicsEnrich
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes XomicsEnrich wait for user response (see UIRESUME)
% uiwait(handles.figure1);


%%%%%%%%%%%%%%%%%%%%%%%%%%% output function %%%%%%%%%%%%%%%%%%%%%%%%%%
% --- Outputs from this function are returned to the command line.
function varargout = XomicsEnrich_OutputFcn(~, ~, handles) 
...
disp('Loading PPI ...');current_file = mfilename('fullpath');   % 获取当前运行代码的地址
[current_path, ~, ~] = fileparts(current_file);[parent_path, ~, ~] = fileparts(current_path);  %获取上一层地址
...
PLSboot_gene_PPI = [];influence_mat = zeros(size(PLSboot_gene40_map2whole308));% 设置影响开关为全关闭
for idx = 1: length(handles.protein_matrix.symbols)
    ...
    PPI_string = unique(union(PPI.protein1(idx_ppi),PPI.protein2(idx_ppi)));% 根据STRING库找到与当前蛋白关联的基因
    [~,act_idx,~,] = intersect(string(handles.gene_matrix.symbols),string(PPI_string));
    influence_mat(act_idx,idx) = 1;% 打开蛋白对这些基因的影响开关
    PLSboot_gene_PPI(:,idx) = influence_mat(:,idx).*PLSboot_gene40_map2whole308(:,idx+1);% 写入影响的效应值
end
...
for idx = 1: length(candidate_proteins)
    ...
    PPI_string = unique(union(PPI.protein1(idx_ppi),PPI.protein2(idx_ppi)));% 根据STRING库找到与当前蛋白first-stage关联的基因
    [~,act_idx,~,] = intersect(string(handles.gene_matrix.symbols),string(PPI_string));
%     geneSymbol_PPI = handles.gene_matrix.symbols(act_idx);
    ...
    temp = Method_PLSbootstrap(X_PPI,Y,handles.dim,max_dim,handles.boot,geneSymbol_PPI,handles.out_path,fig_name);% 打开蛋白对这些基因的影响开关
    PLSboot_gene_PPI2(act_idx,idx) = temp.Z;% 写入影响的效应值
end
...
for trait=1:size(Y,2)
    [temp_R,temp_idx] = sort(abs(R(:,trait)));% ascend,升序
    [~,temp_idx] = sort(temp_idx); % rank of origin r-value
    R_sort =[R_sort,temp_R];% 存储相关性矩阵
    R_sort_idx =[R_sort_idx,temp_idx];% 存储相关性排序
end
R_sort_idx(R_P>0.01) = -Inf;% 相关性不显著的指定排序为极大值Inf
...
R_sort_idx(R_P>0.01) = 0;% 相关性不显著的指定排序为0
score2 = R_sort_idx(:,1) + sum(weig'.*(R_sort_idx(:,2:41)),2);% 使用（总数-排序）作为得分
temp = [R(:,1)';(weig'.*R(:,2:41))'];[~,index]=max(abs(temp),[],1);% max for each column
max_R = [];for idx=1:size(temp,2);max_R = [max_R;temp(index(idx),idx)];end %分号等于行合并
...
max_Z_PPI = [];for idx=1:size(temp,2); max_Z_PPI = [max_Z_PPI;temp(index(idx),idx)];end %分号等于行合并
...
R_sort_idx(R_P>0.01) = 0;% 相关性不显著的指定排序为0
score2_PPI = R_sort_idx(:,1) + sum((weig'.*(R_sort_idx(:,2:41))).*PLSboot_gene_PPI,2);% 使用（总数-排序）作为得分
